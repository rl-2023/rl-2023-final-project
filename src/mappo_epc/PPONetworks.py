...
class KAN_ActorNetwork(nn.Module):
    def __init__(self, obs_space_size, action_space_size,device='cuda' if torch.cuda.is_available() else 'cpu'):
        super().__init__()
        self.device=device
        self.policy_layers = nn.Sequential(
            KANLayer(obs_space_size, 64).to(self.device), 
            nn.LayerNorm(64),
            KANLayer(64, 32).to(self.device),
            nn.LayerNorm(32),
...
class KAN_CriticNetwork(nn.Module):
    ...
    def __init__(self, obs_space_size, n_agents,device='cuda' if torch.cuda.is_available() else 'cpu'):
        super().__init__()
        self.device=device
        self.value_layers = nn.Sequential(
            KANLayer(obs_space_size*n_agents, 64).to(self.device), 
            nn.LayerNorm(64),
            KANLayer(64, 32).to(self.device),
            nn.LayerNorm(32),
